...
index = 1
for i in np.random.randint(low = 0, high = labels.shape[0], size = 10):
    file = train_path + labels.iloc[i]['id'] + '.tif'
    img = Image.open(file)
    ax = fig.add_subplot(2, 5, index)
    ax.imshow(img, cmap = 'gray')
    index = index + 1
    ax.set_title(labels.iloc[i].label, fontsize = 18)
plt.tight_layout()
plt.show()


plt.hist(labels['label'], color = 'blue', edgecolor = 'black')
plt.show()
